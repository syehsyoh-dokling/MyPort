...
function PlaneSvg({ active, mode = "fly" }: { active: boolean, mode?: string }) {
  if (!active) return null;

  return (
    <svg className={`plane-svg ${mode}`} viewBox="0 0 900 420" aria-hidden="true">
      <path
              ...
              <>
                {!(currentSlide === 1 && slide.mode === "bus") && (
                  <div className={`journey-visual-item ${currentSlide === 7 && activeFrame.includes("frame-1.png") ? "slide8-frame-one-active" : ""} ${currentSlide === 7 && !activeFrame.includes("frame-1.png") ? "slide8-flight-frame-active" : ""}` }>
                    ...
                    {currentSlide === 8 && (
                      <>
                        <div className="journey-ground-shadow" style={{ position: 'absolute', bottom: '-2px', left: '50%', transform: 'translateX(-50%)', width: '56px', height: '12px', background: 'radial-gradient(ellipse at center, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0) 70%)', borderRadius: '50%', zIndex: -1 }}></div>
                        <style>{`
                          #journey .journey-panel[data-slide-index="8"] {
                            background-position: center 60% !important;
                          }
                          #journey .journey-panel[data-slide-index="8"]::after {
                            content: "";
                            position: absolute;
                            inset: 0;
                            top: 60%;
                    ...
                    {currentSlide === 10 && (
                      <style>{`
                        @keyframes slide10PlaneCurve {
                          0% { opacity: 0; transform: translate(60%, -10%) scale(0.6) rotate(15deg); }
                          20% { opacity: 1; transform: translate(30%, -30%) scale(0.8) rotate(0deg); }
                          100% { opacity: 1; transform: translate(-10%, 15%) scale(1) rotate(-15deg); }
                        }
                        .plane-svg.tsunami-flight {
                          animation: slide10PlaneCurve 2500ms cubic-bezier(.25,.75,.2,1) forwards;
                        }
                      `}</style>
                    )}
                  </div>
                ...
                {!slide.logoWall && notes.length > 0 && currentSlide !== 6 && currentSlide !== 7 && (
                  <div className={`journey-notes ${currentSlide === 5 ? "early-achievement-notes" : ""} ${currentSlide === 6 ? "early-university-notes" : ""} ${currentSlide === 9 ? "early-tsunami-notes" : ""} ${currentSlide === 10 ? "early-return-notes" : ""}`}>
                    {notes.map((note) => {
                      ...
                      const isFlight = currentSlide === 10;
                      ...
                      return (
                        <div
                          ...
                          style={isFlight ? {
                            ...
                            boxShadow: 'none',
                            color: '#fff',
                            textShadow: '0 4px 10px rgba(0,0,0,0.8), 0 2px 4px rgba(0,0,0,0.6)',
                            padding: 0,
                        ...
                        >
                          {!isFlight && <span>{getIcon(note.text)}</span>}
                          <p style={isFlight ? { borderRight: 'none', fontFamily: 'var(--font-mono, monospace)', border: 'none' } : {}}>{displayText}</p>
                        </div>
                      );
                    })}
                  </div>
